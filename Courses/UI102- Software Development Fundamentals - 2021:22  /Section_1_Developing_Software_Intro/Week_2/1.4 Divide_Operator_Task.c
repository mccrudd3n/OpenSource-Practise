...
namespace Hello_World
{
    class Program
    {
        ...
        static double DivideMethod(double input_1, double input_2)
        {
        ...
        {
            Console.WriteLine("Select Operation: Addition (a), Subtract (s), Multiply (m) or Divide (d)");
            string input_operator = Console.ReadLine();

            while (input_operator != "a" || input_operator != "s" || input_operator != "m" || input_operator != "d")
            {
                ...
                else if (input_operator == "d")
                {
                    Console.WriteLine($"{input_1} + {input_2} = {DivideMethod(input_1, input_2)}");
                    break;
                }
            }
        }
